...
public class Helpers
{
	...
	public static boolean playerAttackVictimWithWeapon(EntityPlayer player, Entity entity, ItemStack mainhand, ItemStack offhand, boolean mainhandAttack)
	{
		...
		boolean configWeapon = false;

		ConfigWeaponPotionEffect customWeaponPotionEffect = null;

		/* Shield bash */
		if ( !mainhandAttack && offhand.getItem() instanceof ItemShield )
		{
			weapon = offhand;
			isMetal = isMetalItem(offhand);
			knockbackMod = EnchantmentHelper.getKnockbackModifier(player);
			...
			{
				if ( offhand.getItem().equals(customShield.shield) )
				{
		...
		if ( entity instanceof EntityLivingBase )
		{
			...
			if ( isShield )
			{
				if ( ConfigurationHandler.shieldSilverDamageMultiplier != 1.0F && victim.isEntityUndead() && isSilverRegistryName(getRegistryNameFromItem(offhand)) )
				{
		...
		if ( playerAttackedVictim(player, victim, bodyPart, (float) damage, weapon) )
		{
			...
			/*
			 * SWEEPING Weapons with this property will inflict half attack damage (for level I) or full attack damage (for level II) to all targets that get hit by the
			 * sweep attack
			 */
			...
			if ( sweepAmount-- > 0 )
			{
				...
				for (EntityLivingBase sweepVictim : sweepVictims)
				{
					...
					if ( sweepVictim != player && sweepVictim != victim && sweepVictim.isEntityAlive() && player.getDistanceSq(sweepVictim) <= (additionalReach + sweepVictim.width) * (additionalReach + sweepVictim.width) && victim.getDistanceSq(sweepVictim) <= (sweepRange) * (sweepRange) && ConfigurationHandler.rightClickAttackable(player, sweepVictim) && !player.isOnSameTeam(sweepVictim) )
					{
						...
						sweepDamage = MathHelper.clamp(damage * (sweepAmount * 0.25D), ConfigurationHandler.baseAttackDamage, damage);
...
//		try{knockbackMod += player.getActivePotionEffect(MobEffects.STRENGTH).getAmplifier();}catch (Exception e){}
//		try{knockbackMod -= player.getActivePotionEffect(MobEffects.WEAKNESS).getAmplifier();}catch (Exception e){}
//		
//		for ( CustomShield customShield : ConfigurationHandler.shields )
//		{
